    ...
    <el-table :data="table.records || []" stripe>
      ...
      <el-table-column label="风险等级">
        <template #default="scope">
          <span>{{ scope.row.role === 'STUDENT' ? (scope.row.riskLevel || '-') : '-' }}</span>
        </template>
      </el-table-column>
...
const load = () => request.get('/api/v1/admin/user', {params: query}).then(res => Object.assign(table, res.data || {}))
...
const del = (id) => request.delete('/api/v1/admin/user/' + id).then(() => {
  ElMessage.success('删除成功')
  if ((table.records || []).length === 1 && query.pageNum > 1) {
    query.pageNum -= 1
  }
  load()
})
