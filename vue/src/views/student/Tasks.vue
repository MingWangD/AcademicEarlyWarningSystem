  ...
  <el-card>
    ...
    <el-table :data="tasks" stripe>
      ...
      <el-table-column label="操作" width="180">
        <template #default="scope">
          <el-button v-if="scope.row.type === 'homework'" size="small" type="primary" :disabled="scope.row.status==='已提交'" @click="submitHomework(scope.row)">提交作业</el-button>
          ...
          <el-button v-else size="small" type="warning" :disabled="scope.row.status==='已完成'" @click="submitExam(scope.row)">提交考试</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-card>
...
const submitHomework = (row) => request.post('/api/v1/student/homework/submit', {homeworkId: row.taskId, content: '已按要求完成'}).then(()=>{ElMessage.success('作业已提交');load()})
...
const submitExam = (row) => request.post('/api/v1/student/exam/submit', {examId: row.taskId, answers: {q1:'A', q2:'C'}}).then(res=>{ElMessage.success(`考试提交成功，得分 ${res.data.score}`);load()})
